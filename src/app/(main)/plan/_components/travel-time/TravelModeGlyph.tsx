...
export function TravelModeGlyph({ mode }: { mode: string }) {
  ...
  if (key.includes("WALK") || key === "WALKING")
    return <Footprints className="h-4 w-4 shrink-0 text-brand-red" />;
  if (
    key.includes("DRIVE") ||
    key.includes("CAR") ||
    key === "DRIVING"
  )
    return <Car className="h-4 w-4 shrink-0 text-brand-red" />;
  if (key.includes("TRANSIT") || key.includes("BUS") || key.includes("SUBWAY"))
    return <Bus className="h-4 w-4 shrink-0 text-brand-red" />;
  if (key.includes("CYCL") || key === "CYCLING" || key.includes("BICY"))
    return <Bike className="h-4 w-4 shrink-0 text-brand-red" />;
  return <Footprints className="h-4 w-4 shrink-0 text-brand-red" />;
}
